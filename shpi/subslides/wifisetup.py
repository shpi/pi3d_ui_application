...
def inloop(x = 0, y = 0, touch_pressed = False, textchange = False,activity = False):
    ...
    if wifinetworks == None:
        ...
        for network in wifinetworks:
            ...
            #TODO not good to create FixedString in loop
            wifinetworks[actnetwork]['string'] = pi3d.FixedString(font_path,'SSID: '+(str)(network['essid']) + ', Enc:' + (str)(network['enc']) + '  Ch:' + (str)(network['ch']) , font_size=32,shadow_radius=4,justify='L', background_color=(0,0,0,30), color= (255,255,255,255),camera=graphics.CAMERA, shader=graphics.SHADER, f_type='SMOOTH')
