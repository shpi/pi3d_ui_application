...
class EgClass(object):
  usertext = (str)('')
  usertextshow = 'Please enter WIFI password.' 
eg_object = EgClass()
...
wifistatus = pi3d.PointText(pointFont, CAMERA, max_chars=320, point_size=128)    #slide 1
...
textblock = pi3d.TextBlock(-370, 195, 0.1, 0.0, 27, data_obj=eg_object,attr="usertextshow",text_format= "{:s}", size=0.29, spacing="F", space=0.05, colour=(1.0, 1.0, 1.0, 1.0))
keyboardslide.add_text_block(textblock)


textblock = pi3d.TextBlock(-370, 195, 0.1, 0.0, 170, data_obj=eg_object,attr="ssid",text_format= "SSID: {:s}", size=0.29, spacing="F", space=0.05, colour=(1.0, 1.0, 1.0, 1.0))
wifistatus.add_text_block(textblock)

textblock = pi3d.TextBlock(-370, 95, 0.1, 0.0, 27, data_obj=eg_object,attr="wifistrength",text_format= "Signal: {:s}", size=0.29, spacing="F", space=0.05, colour=(1.0, 1.0, 1.0, 1.0))
wifistatus.add_text_block(textblock)

textblock = pi3d.TextBlock(-370, -5, 0.1, 0.0, 47, data_obj=eg_object,attr="ipaddress",text_format= "IP: {:s}", size=0.29, spacing="F", space=0.05, colour=(1.0, 1.0, 1.0, 1.0))
wifistatus.add_text_block(textblock)


wifinetworkstext = pi3d.FixedString('opensans.ttf', 'Please choose your WIFI-network' , font_size=42, shadow_radius=4,justify='L', background_color=(0,0,0,30), color= (255,255,255,255),camera=CAMERA, shader=shader, f_type='SMOOTH')
  ...
  if subslide == 'wifisetup': 
       ...
       if wifinetworks == None:
        ...
        for network in wifinetworks:
         ...
         wifinetworks[actnetwork]['string'] = pi3d.FixedString('opensans.ttf', (str)(network['essid']) + ' Enc:' + (str)(network['enc']) + '  Ch:' + (str)(network['ch']) , font_size=32,shadow_radius=4,justify='L', background_color=(0,0,0,30), color= (255,255,255,255),camera=CAMERA, shader=shader, f_type='SMOOTH')
         ...
         if touch_pressed:
           touch_pressed = False
           calculateselectednetwork = abs((int)((lasty - 100)/80))
           if -1 <  calculateselectednetwork  <  len(wifinetworks):
              eg_object.usertextshow = 'Please enter WIFI password.'
              subslide = 'wifipassword'

  if subslide == 'wifipassword':
     if touch_pressed:
      touch_pressed = False
      if 150 < lasty <  240:
        subslide = 'wifistatus'
