...
#to disable function set relay to 0
coolingrelay = 1 #off
heatingrelay = 3 #on relay3

shutterdown = 1  #on
shutterup = 4    # 4... buzzer, 5 d13, 6 hwb

lasttouch = 0


def controlrelays(channel, value):
 try:
  if channel == 1: #relais1
   if value == 1:
     bus.write_byte_data(ADDR_32U4,0x8D,0xFF)
   else:
     bus.write_byte_data(ADDR_32U4,0x8D,0x00)

  elif channel == 2: #relais2
   if value == 1:
     bus.write_byte_data(ADDR_32U4,0x8E,0xFF)
   else:
     bus.write_byte_data(ADDR_32U4,0x8E,0x00)

  elif channel == 3: #relais3
   if value == 1:
...
## cooling & heating function
    ...
    if ADDR_32U4:
     ...
     if read_one_byte(0x0D) == 255: eg_object.relais1 = 1 
     else: eg_object.relais1 = 0

     if read_one_byte(0x0E) == 255: eg_object.relais2 = 1 
     else: eg_object.relais2 = 0

     if read_one_byte(0x0F) == 255: eg_object.relais3 = 1 
     else: eg_object.relais3 = 0

     if read_one_byte(0x10) == 255: eg_object.d13 = 1 
     else: eg_object.d13 = 0

     if read_one_byte(0x11) == 255: eg_object.hwb = 1 
     else: eg_object.hwb = 0

     if read_one_byte(0x12) == 255: eg_object.buzzer = 1 
     else: eg_object.buzzer = 0
...
"LINE2:bmp280#888800:BMP280")
  ...
  if textchange == True:
     if coolingrelay:
      if eval('eg_object.relais' + (str)(coolingrelay)):  # dangerous, just for testing
        cooling.colouring.set_colour([0,0,1])
      else:
         cooling.colouring.set_colour([1,1,1])
     if heatingrelay:
      if eval('eg_object.relais' + (str)(heatingrelay)):  # dangerous, just for testing
